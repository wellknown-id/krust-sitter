...
impl TokenCollector {
    ...
    /// Generate a JavaScript preview highlighter that uses the exact same
    /// regex patterns as the TextMate grammar.
    fn to_preview_js(&self, lang_name: &str) -> String {
        let mut js = String::new();
        js.push_str("// Auto-generated by krust-sitter — do not edit\n");
        js.push_str("(function () {\n");

        // Build ordered token patterns (comments first for priority)
        js.push_str("  var patterns = [\n");

        // Comments
        for cp in &self.comment_patterns {
            match cp {
                CommentPattern::Line { pattern, .. } => {
                    let escaped = js_regex_escape(pattern);
                    js.push_str(&format!(
                        "    {{ regex: /{}/gm, cls: '{lang_name}-comment' }},\n",
                        escaped
                    ));
                }
                CommentPattern::Block { begin, end, .. } => {
                    let escaped = format!(
                        "{}[\\s\\S]*?{}",
                        js_regex_escape(begin),
                        js_regex_escape(end)
                    );
                    js.push_str(&format!(
                        "    {{ regex: /{}/g, cls: '{lang_name}-comment' }},\n",
                        escaped
                    ));
                }
            }
        ...
        let strings: Vec<&CollectedToken> = self
            ...
            .collect();
        for t in &strings {
            let escaped = js_regex_escape(&t.pattern);
            js.push_str(&format!(
                "    {{ regex: /{escaped}/g, cls: '{lang_name}-string' }},\n"
            ));
        }

        // Operators that start with @ (decorators) — extract before keywords
        let decorators: Vec<&CollectedToken> = self
            .tokens
            .iter()
            .filter(|t| t.category == TokenCategory::Operator && t.pattern.starts_with('@'))
            .collect();
        if !decorators.is_empty() {
            let dec_pattern = decorators
                .iter()
                .map(|t| js_regex_escape(&regex_escape(&t.pattern)))
                .collect::<Vec<_>>()
                .join("|");
            js.push_str(&format!(
                "    {{ regex: /{dec_pattern}/g, cls: '{lang_name}-decorator' }},\n"
            ));
        }
        ...
        for t in &numerics {
            let escaped = js_regex_escape(&t.pattern);
            js.push_str(&format!(
                "    {{ regex: /{escaped}/g, cls: '{lang_name}-number' }},\n"
            ));
        }
...
"#,
    ...
    /// Generate CSS for the preview highlighter.
    fn to_preview_css(&self, lang_name: &str) -> String {
        // Use VS Code dark-theme-compatible colors
        format!(
            r#"/* Auto-generated by krust-sitter — do not edit */
.{lang_name}-keyword {{ color: #c586c0; }}
.{lang_name}-comment {{ color: #6a9955; font-style: italic; }}
.{lang_name}-string {{ color: #ce9178; }}
.{lang_name}-number {{ color: #b5cea8; }}
.{lang_name}-decorator {{ color: #dcdcaa; }}
"#
        )
    }
}

/// Generated preview files (JS + CSS) for markdown preview highlighting.
...
pub fn generate_preview(grammar: &Grammar, scope_name: Option<&str>, lang_name: &str) -> PreviewFiles {
    ...
    PreviewFiles {
        js: collector.to_preview_js(lang_name),
        css: collector.to_preview_css(lang_name),
    }
}
...
/// Forward slashes must be escaped as they delimit the literal.
